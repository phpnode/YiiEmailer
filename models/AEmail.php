...
class AEmail extends CActiveRecord {
	...
	public function rules() {
		return array(
			array("sender,recipient","required"),
		);
	}
	...
	public function send($runValidation = true) {
		...
		return Yii::app()->email->getMailer()->send($this);
	}
	/**
	...
	protected function renderPartial($viewName, $data = array()) {
		...
		else if (is_object(Yii::app()->controller)) {
	...
	/**
	 * Get a string representation of the email
	 * @return string the email details
	 */
	public function __toString() {
		$text = $this->headers."\n";
		$text .= "Subject: ".$this->subject."\n";
		$text .= "From: ".$this->sender."\n";
		$text .= "To: ".$this->recipient."\n";
		$text .= "\n";
		$text .= $this->render();
